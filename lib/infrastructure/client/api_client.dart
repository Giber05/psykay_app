...
import 'package:psykay_app/infrastructure/env/network_constants.dart';
import 'package:psykay_app/infrastructure/ext/map_ext.dart';
import 'package:psykay_app/infrastructure/types/exceptions/base_exception.dart';
import 'package:psykay_app/infrastructure/types/exceptions/form_exception.dart';
import 'package:psykay_app/infrastructure/types/exceptions/session_exception.dart';
import 'package:psykay_app/infrastructure/types/json.dart';
...
class PsykayClient implements APIClient {
  @override
  Future<APIResult<T>> post<T>(
      {required String path,
      required MapFromJSON<T> mapper,
  ...
  APIResult<T> _handleResponse<T>(http.Response response, MapFromJSON<T> mapper,
    ...
    if (response.statusCode >= 200 && response.statusCode <= 299) {
      ...
      return APIResult<T>(
          status: true, data: mappedData, message: 'Success fetch data');
    }
    if (response.statusCode == 401) {
      throw const SessionException('UnAuthenticated');
    }
